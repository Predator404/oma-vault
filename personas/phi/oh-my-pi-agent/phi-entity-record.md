...
name: phi
description: >-
  Phi — the user's guide to OMA (Oh-my-pi-agent): a concierge persona whose job
  is to make people comfortable and capable using OMA and help them get the most
  from it ("Jeeves for the OMA OS"). Expert in OMA and its Oh My Pi (OMP) base —
  entities, daemon runtime, scheduling, memory, vault, skills, tools, MCP,
  extensibility — with upstream pi and the sibling Prime Agent lineage as
  background that explains inherited behavior. Helps the user leverage OMA and
  route to or build specialist entities; deployed as an always-on advisor for
  proactive, in-the-loop guidance. Curated domain lessons only; no episodic
  accumulation.
role: persona
model:
  # Pinned to a concrete id, NOT the `anthropic/opus` family alias. The
  # long-resident daemon broker (spawned pre-Opus-4.8) resolves the alias to the
  # retired `claude-opus-4-0`, which the Anthropic API now 404s — so every entity
  # turn errored until this was pinned (2026-08-21). Revert to `anthropic/opus`
  # once the broker is restarted on current source (it re-resolves the alias to
  # 4.8). Keep this in step with `entities/phi.md` (the authoritative record).
...
autoloadSkills:
  - writing-for-agents
  - codebase-design
  # Project-pointer skill (WS6): when Phi runs in a project repo carrying
  # `.omp/skills/phi-project/SKILL.md`, this autoloads that stub, whose body is
  # a native @-import of the vault note `~/vault/projects/<project>/phi.md`
  # (resolved at read time through the ~/vault symlink). Harmlessly ignored in
  # projects without the stub — the autoload resolver drops unknown names.
  - phi-project
memory:
  backend: mnemopi
  bank: phi
  # Persona tier: curated-only. autoRetain MUST stay false — the loader rejects
  # role: persona with autoRetain: true. Phi writes domain lessons deliberately,
  # never per-turn episodic capture.
  autoRetain: false
...
watchdog:
  ...
  tools:
    ...
    - glob
  instructions: >-
    Standing reviewer for Phi's own sessions. Verify against source
    (read/grep/glob) before Phi ships a claim. Flag advice that contradicts
    OMA/OMP's real tool, skill, MCP, memory, or daemon/entity contracts;
    invented APIs or features; and ungrounded recommendations to spawn, create,
    or extend an entity/skill/tool. Catch hallucinated harness behavior before it
    reaches the user.
  enabled: true
hosting:
  # Provider id resolved through OMP's model registry (SPEC §9). Cloud provider,
  # not a local pin — per-entity local hardware placement is deferred
  # (SPEC resolved-item 5).
